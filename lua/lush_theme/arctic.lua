...
local theme = lush(function(injected_functions)
	...
	return {
		...
		sym("@type.definition")({ fg = blue_green, gui = "italic" }), -- identifiers in type definitions (e.g. `typedef <type> <identifier>` in C)
		...
		-- Function
		sym("@function")({ Function, gui = "italic" }), -- function definitions
		...
		sym("@function.method")({ sym("@function"), gui = "italic" }), -- method definitions
		...
		-- Keyword
		sym("@keyword")({ Keyword, gui = "italic" }), -- keywords not fitting into specific categories
		sym("@keyword.coroutine")({ fg = dark_pink, gui = "italic" }), -- keywords related to coroutines (e.g. `go` in Go, `async/await` in Python)
		sym("@keyword.function")({ fg = dark_blue, gui = "italic" }), -- keywords that define a function (e.g. `func` in Go, `def` in Python)
		sym("@keyword.operator")({ sym("@operator"), gui = "italic" }), -- operators that are English words (e.g. `and` / `or`)
		sym("@keyword.import")({ Include, gui = "italic" }), -- keywords for including modules (e.g. `import` / `from` in Python)
		sym("@keyword.type")({ fg = dark_blue, gui = "italic" }), -- keywords describing composite types (e.g. `struct`, `enum`)
		sym("@keyword.modifier")({ fg = dark_blue, gui = "italic" }), -- keywords modifying other constructs (e.g. `const`, `static`, `public`)
		sym("@keyword.repeat")({ Repeat, gui = "italic" }), -- keywords related to loops (e.g. `for` / `while`)
		sym("@keyword.return")({ fg = dark_pink, gui = "italic" }), --  keywords like `return` and `yield`
		sym("@keyword.debug")({ Debug, gui = "italic" }), -- keywords related to debugging
		sym("@keyword.exception")({ Exception, gui = "italic" }), -- keywords related to exceptions (e.g. `throw` / `catch`)

		sym("@keyword.conditional")({ Conditional, gui = "italic" }), -- keywords related to conditionals (e.g. `if` / `else`)
		sym("@keyword.conditional.ternary")({ sym("@operator") }), -- ternary operator (e.g. `?` / `:`)

		sym("@keyword.directive")({ PreProc, gui = "italic" }), -- various preprocessor directives & shebangs
